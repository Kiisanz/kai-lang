...
mod tests {
    ...
    fn test_optional_type() {
        ...
        assert_eq!(var_decl.declared_type, Some(Type::Optional(Box::new(Type::Int32))));
        assert_eq!(var_decl.initializer, None);
    }
    ...
    fn test_no_type_no_init_error() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().message.contains("must have either a type annotation or an initializer"));
    }
